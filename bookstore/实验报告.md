...
### 5.1 分支策略与关键提交
- 工作在 `clean-master` 分支：清理大文件、更新 `.gitignore` 后进行开发。
- 主要阶段：
1. MongoDB 模型替换 SQLite，并保持接口兼容。
2. 实现扩展功能（发货 / 收货 / 搜索 / 订单查询）。
3. 编写辅助脚本（性能基准、实验样例）。
4. 补充单元测试，覆盖率提升到 93%。
5. 整理实验报告素材。

### 5.2 问题与解决
- **Git 大文件**：初次提交带有大型 SQLite，导致 push 失败 → 清理文件、更新 `.gitignore`。
- **索引脚本**：`Cursor.explain` 参数不兼容 → 修正 PyMongo 版本调用方式。
- **索引性能展示**：通过 `book_lx` 数据集 + 自定义查询参数，确保索引对比明显。
